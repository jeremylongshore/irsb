...
For large APIs where token efficiency is critical, MAPI specifications can be authored as a modular skill structure with an index file, shared definitions, and individual capability files. This enables agents to load only the capabilities they need.
